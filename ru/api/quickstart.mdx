---
title: "Быстрый старт"
description: "Создайте, подпишите и отправьте свою первую транзакцию продажи менее чем за пять минут, затем настройте поток автоматической продажи."
---

## Предварительные требования

- API ключ LaserSell ([получите здесь](/api/authentication)).
...
<Tip>
**Используете ИИ для кода?** Добавьте [MCP сервер LaserSell](/ai-agents/mcp-server) в ваш редактор, чтобы ИИ ассистент мог искать документацию LaserSell в реальном времени. Работает с Claude Code, Claude Desktop, Cursor и Windsurf.
</Tip>
...
- [POST /v1/sell](/api/exit-api/sell): полная схема запроса и ответа для транзакций продажи.
- [Exit Intelligence Stream](/api/stream/overview): детальное описание WebSocket потока.
- [Подпись транзакций](/api/transactions/signing): некастодиальный процесс подписи.
