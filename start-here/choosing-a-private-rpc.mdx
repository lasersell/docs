...
- **Why:** This combination avoids the limitations of using a single free provider. Helius offers superior transaction landing on their RPCs, while Chainstack provides a stable and generous websocket connection on their free tier, ensuring you can stream market data without hitting immediate rate limits.

<Warning>
  This "Free Route" strategy should only just be to "try it out". Free tier WebSocket connections have a tendency to stop responding and are not optimal. It's highly recommended to get a paid plan from an RPC provider like Helius and use their WebSocket servers.
</Warning>
