---
title: "設定例"
description: "すべての利用可能なフィールド、デフォルト値、環境変数オーバーライドを含む注釈付きconfig.ymlリファレンス。"
---

セットアップウィザードが`~/.lasersell/config.yml`を生成します。直接編集してデーモンを再起動して変更を適用するか、`lasersell --setup`を再実行してください。
...
| `account.send_target` | `LASERSELL_SEND_TARGET` | `helius_sender`、`astralane`、または`rpc` |
| `account.astralane_api_key` | `LASERSELL_ASTRALANE_API_KEY` | `ast_xyz789...` |
| ウォレットパスフレーズ | `LASERSELL_WALLET_PASSPHRASE` | （ヘッドレスアンロック用） |
...
| ターゲット | 説明 |
|---|---|
| `helius_sender` | デフォルト。Heliusの最適化された送信者を使用。Helius RPC URLが必要。 |
| `astralane` | Astralaneの送信サービスを使用。`astralane_api_key`が必要。 |
| `rpc` | RPCエンドポイント経由で直接送信。任意のプロバイダーで動作しますが、ランディングが遅くなる可能性があります。 |
