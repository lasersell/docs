...
| `lasersell_sdk::stream::client` | `StreamClient`, `StreamConnection`, `StreamSender` |
    ...
    loop {
        ...
        match &event {
            StreamEvent::ExitSignalWithTx { message, .. } => {
                ...
                let sig = send_transaction(&SendTarget::HeliusSender, &signed).await?;
...
let sig = send_transaction(&SendTarget::HeliusSender, &signed_tx).await?;
