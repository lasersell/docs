...
- **LaserSell API** (REST): Build unsigned buy and sell transactions on demand via `POST /v1/sell` and `POST /v1/buy`. Retrieve your account details with `GET /v1/account` and query your trade history with `GET /v1/history`.
