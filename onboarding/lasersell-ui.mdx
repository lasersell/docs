...
- `set tp <percent>` — target profit
- `set sl <percent>` — stop loss
- `set ts <percent>` — trailing stop (alias: `set trailing <percent>`)
- `set slippage <percent>` — max slippage (alias: `set slip <percent>`)
- `set timeout <seconds>` — deadline timeout (alias: `set to <seconds>`)
- `set graduation <on|off>` — sell on graduation
...
- `tp`, `sl`, and `ts` require a percent string like `6%` (include the `%`).
...
- `graduation` accepts `on`/`true` or `off`/`false`.

All `set` commands write to your config file automatically and apply live — no restart required.
