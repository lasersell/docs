...
At least one exit condition (target profit, stop loss, trailing stop, or deadline) must be enabled. You can change any of these later from the TUI or by editing `~/.lasersell/config.yml`.
