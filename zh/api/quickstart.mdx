---
title: "快速入门"
description: "在五分钟内构建、签名和提交你的第一笔卖出交易，然后设置自动卖出流。"
---

## 前提条件

- 一个 LaserSell API 密钥（[在此获取](/api/authentication)）。
- 一个 Solana 密钥对文件（JSON 字节数组）。
- Node.js 18+（TypeScript 示例）或你首选语言的运行时。

<Tip>
**使用 AI 编程？** 将 [LaserSell MCP 服务器](/ai-agents/mcp-server)添加到你的编辑器，让你的 AI 助手能够实时搜索 LaserSell 文档。兼容 Claude Code、Claude Desktop、Cursor 和 Windsurf。
</Tip>

## 示例 1：构建、签名和发送卖出交易

此示例调用 LaserSell API 构建未签名的卖出交易，在本地签名，并通过 Helius Sender 提交。
...
连接退出智能流以监控钱包，当策略阈值达到时自动执行退出。

<Warning>
**在买入之前连接流。** 流通过监控链上代币到达来检测仓位。如果你在流连接和配置之前调用 `/v1/buy`，仓位将不会被追踪。始终先启动流，然后提交买入交易。
</Warning>
...
- [POST /v1/sell](/api/exit-api/sell)：卖出交易的完整请求和响应模式。
- [退出智能流](/api/stream/overview)：WebSocket 流深度解析。
- [交易签名](/api/transactions/signing)：了解非托管签名流程。
